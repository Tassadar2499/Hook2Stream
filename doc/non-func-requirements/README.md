...
- один поддерживаемый MP3 или WAV длительностью 3–5 минут;
- один cover и пять image/video assets;
- не более 500 MB входных visual assets суммарно;
- lyrics либо подтверждённый `Instrumental`;
- три hooks, один preview и Release Pack из 18 роликов;
- без generative video backgrounds и неподдерживаемых codecs.
...
| `NFR-PERF-003` | Валидный reference project должен перейти от `upload complete` до готового storyboard с P50 не более 10 минут и P95 не более 20 минут. | Synthetic project измеряет единый operation timestamp от подтверждения всех uploads до сохранённого `CampaignPlan`; queue time включается. |
...
| `NFR-PERF-006` | Изменение job state должно появляться в UI не позднее 5 секунд после commit; после reconnect актуальное состояние должно восстанавливаться не позднее 10 секунд. | SSE и polling-fallback tests разрывают соединение, меняют state и измеряют время до согласованного UI. |
...
| `NFR-REL-004` | Потерянный worker lease должен быть обнаружен, а незавершённая retryable operation возвращена в очередь не позднее 5 минут после lease expiry. | Chaos test завершает worker во время analysis, render и export; logical job восстанавливается без ручного изменения БД. |
...
| `NFR-REL-007` | Production paid beta должна использовать single-region topology с минимум двумя control-plane instances за managed load balancer, managed PostgreSQL с PITR и S3-compatible object storage. Analysis и render workers развёртываются отдельными pools. | Architecture review и deployment smoke test подтверждают отсутствие single process для API и media work; rolling deploy не прерывает synthetic control-plane check. |
| `NFR-REL-008` | Production capacity и SLO не должны зависеть от домашнего компьютера, домашнего GPU или вручную запущенного worker. Kubernetes не является обязательным условием. | Отключение всех developer machines не меняет readiness production; inventory перечисляет только управляемые production resources. |
...
| `NFR-DATA-002` | Originals, derived assets, renders и bundles должны иметь content hash и проверяться после upload, render, copy и restore. Повреждённые bytes не должны переходить в `Ready`. | Corruption test изменяет object bytes; integrity validation отклоняет результат и создаёт диагностируемый retry/failure. |
...
| `NFR-DATA-006` | Неоплаченный project вместе с assets и free preview должен удаляться после 30 дней неактивности; originals, proxies и preview оплаченного project — через 90 дней после готовности последнего paid Pack; paid final outputs, manifests и bundles — через 365 дней. | Time-shift tests проверяют обе стороны каждого boundary; UI и Terms показывают срок до необратимого cleanup. |
...
| `NFR-SEC-003` | Secrets должны поступать только из approved secret manager/runtime secret store и не попадать в repository, image layers, logs, traces, browser bundle или export. Long-lived credentials ротируются не реже одного раза в 90 дней и немедленно при подозрении на compromise. | Secret scanning выполняется в CI; rotation drill меняет credential без rebuild и без потери незавершённых jobs. |
...
| `NFR-SEC-005` | Media workers должны работать non-root в изолированном process/container context с read-only base filesystem, временным рабочим каталогом, CPU/memory/disk/time limits и deny-by-default egress, кроме allowlisted storage/telemetry dependencies. | Sandbox test пытается записать вне workdir, превысить limits и обратиться к внешнему host; API/control plane сохраняет работоспособность. |
...
| ID | Требование | Проверка |
|---|---|---|
| `NFR-OBS-001` | OpenTelemetry trace должен связывать browser/API request, durable job, .NET orchestrator, Python analysis, Node render и object-storage operation единым trace context. | End-to-end synthetic project открывается как связный trace без разрыва на queue boundary; async links сохраняют logical operation ID. |
| `NFR-OBS-002` | Application logs должны быть structured и не содержать lyrics, media bytes, credentials, payment payloads, auth tokens или presigned URLs. Production operational logs хранятся 30 дней. | Automated redaction tests отправляют sentinel values; log scan не находит их и подтверждает lifecycle policy. |
...
| `NFR-UX-005` | Полный workflow должен работать при viewport от 360 px, в portrait/landscape и с touch input без потери данных или обязательного перехода на desktop. | Responsive Playwright suite проходит signup → upload → hooks → storyboard → checkout → download на mobile/tablet/desktop profiles. |
...
| ID | Требование | Проверка |
|---|---|---|
| `NFR-ENG-001` | `SongAnalysis`, `CompositionSpec`, `CampaignPlan` и export manifest должны иметь explicit schema version. Workers обязаны поддерживать текущую и предыдущую production version для завершения in-flight jobs. | Contract fixtures обеих versions читаются новым deployment; неизвестная version отклоняется до side effect с понятным error code. |
...
| `NFR-ENG-003` | CI release gate должен включать unit, integration с PostgreSQL/S3, .NET↔Python↔Node contract tests, golden-media smoke, security scans и основной Playwright flow. | Protected branch не создаёт deployable artifact при failure обязательного suite; report хранит версии tools и fixtures. |
...
| `CFG-WORKER-*` | Lost lease recovery ≤ 5 min; concurrency и resource limits задаются отдельно для analysis/render/export |
