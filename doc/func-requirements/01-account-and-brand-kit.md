...
| `FR-ACC-005` | Public landing должен показывать оффер `One song. Three weeks of ready-to-post lyric shorts`, фактический результат из 18 роликов, обязательные входы, тарифы и отсутствие обещания вирусности. | Посетитель до регистрации может понять количество роликов, 21-дневный результат, бесплатный preview и цены; landing не рекламирует автопубликацию или generative video как MVP. |
...
| `FR-BRD-003` | Если пользователь не задал палитру или fonts, система должна сформировать редактируемые безопасные defaults из обложки и поддерживаемого font allowlist. | Проект только с обложкой получает читаемую палитру и fonts до campaign generation; пользователь может изменить каждый default. |
...
| `FR-BRD-005` | Сохранённый brand kit должен переиспользоваться в следующих релизах, но изменение глобального kit не должно незаметно менять уже созданный campaign plan. | Новый project получает актуальные defaults; существующий plan продолжает ссылаться на свой snapshot до явного `Apply updated brand kit`. |
